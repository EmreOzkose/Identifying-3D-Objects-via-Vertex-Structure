...
void main()
{
	...
	vec4 pos=vPosition;
	float mtime=time*1.5f;
	pos.y = vPosition.y+ rand2(vec2(vPosition.x,vPosition.z))*sin(mtime + rand(vec2(vPosition.x,vPosition.z))*  vPosition.x)+ rand2(vec2(vPosition.x,vPosition.z))*sin(mtime +rand(vec2(vPosition.x,vPosition.z))*  vPosition.z);
