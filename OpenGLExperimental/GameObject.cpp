...
void GameObject::load_obj(string path,GLfloat scale)
{
	string line, v, X, Y, Z;
	vec4 vertex;
	ifstream file(path);
	...
	for (size_t i = 0; i < vertexList.size(); i++)
	{
		...
		EndVertexPositions.push_back(vec4(vec3(v.x,v.y,v.z)*scale,v.w));
...
void GameObject::SetupMesh()
{
	...
	//albedo
	if (go_texture.albedo.c_str() == "")
		return;
	int width, height, nrChannels;
	cout << go_texture.albedo.c_str() << endl;
...
void GameObject::Scale(GLfloat val)
{

	for (size_t i = 0; i < EndVertexPositions.size(); i++)
	{
		EndVertexPositions.at(i) = EndVertexPositions.at(i)*val;
		EndVertexPositions.at(i).w = 1;
